...
class ClienteService {
  ...
  async crearCliente(nombreCliente, correoCliente) {
    try {
      ...
      if (response.status == 201) {
        ...
        toastExito(i18n.global.t('clientes.creado', { cliente: toTitleCase(nombreCliente) }))
        return true
      } else return false
    } catch (error) {
      console.log(error)
  ...
  async editarCliente(nombreCliente, correoCliente, idCliente) {
    try {
      ...
      if (response.status == 200) {
        toastExito(i18n.global.t('clientes.editado', { cliente: toTitleCase(nombreCliente) }))
        return true
      } else return false
    } catch (error) {
      console.log(error)
  ...
  async fetchPorEmpresa(empresa) {
    try {
      const urlClientes = `${this.utils.urlApi}/clientes/search/correo?txt=${empresa}&size=1000`
      const response = await this.utils.fetchConToken(urlClientes, 'GET', null)
      const json = await response.json()
      this.clientes.value = await json._embedded.clientes
      return response.status == 200 ? true : false
  ...
  async deleteCliente(idCliente) {
    try {
      ...
      this.cliente.value = await json
      if (response.status == 200) {
        toastExito(i18n.global.t('clientes.clienteBorrado', { cliente: toTitleCase(this.cliente.value.nombre) }))
        return true
      } else return false    
    } catch (error) {
      console.log(error)
